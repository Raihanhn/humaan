...
const FallingText = ({
  ...
  useEffect(() => {
    ...
    // Remove pointer-event blocking to allow page scroll
    render.canvas.style.pointerEvents = "auto";

    const mouseConstraint = MouseConstraint.create(engine, {
      mouse,
      constraint: {
        stiffness: mouseConstraintStiffness || 0.2,
        angularStiffness: 0,
        render: { visible: true },
      },
    });

    // Allow scrolling while not dragging
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 z-10" ref={canvasContainerRef} />
    </div>
  );
};
